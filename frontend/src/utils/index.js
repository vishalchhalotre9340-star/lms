...
export function getSidebarLinks(forMobile = false) {
	...
	return filterStudentSidebarLinks(links)
}
...
const filterStudentSidebarLinks = (links) => {
	if (!isPureStudent()) return links

	const studentLinks = new Set(['Dashboard', 'Courses', 'Batches'])
	return links
		.map((link) => ({
			...link,
			items: link.items.filter((item) => studentLinks.has(item.label)),
		}))
